...
import matplotlib.pyplot as plt # [NEW] Import để vẽ biểu đồ
...
class WGAN(object):
    ...
    def train(self, trainingset, validationset):
        ...
        epoch_iterator = self._get_epoch_iterator()
                # [NEW] Danh sách lưu lịch sử loss
        d_losses = []
        g_losses = []
        for epoch in epoch_iterator:
            ...
            for c in range(self.critic_iter):
                ...
                discriminated_normal = torch.mean(self.discriminator(normal_traffic)).view(1)
            ...
            self.optim_G.step()
            print(f"Epoch [{epoch}/{self.epochs}], D Loss: {discriminator_loss.item():.6f}, G Loss: {generator_loss.item():.6f}")

            # [NEW] Lưu lịch sử loss
    ...
    def plot_losses(self, d_losses, g_losses):
        ...
        plt.plot(d_losses, label="Critic (D) Loss", color='red', alpha=0.7)
        plt.plot(g_losses, label="Generator (G) Loss", color='blue', alpha=0.7)
        plt.xlabel("Epochs")
        plt.ylabel("Loss")
        plt.title(f"Training Loss - {self.name}")
        plt.legend()
        plt.grid(True)
        ...
        save_path = os.path.join(save_dir, f'{self.name}_loss_plot.png')
